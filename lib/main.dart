...
class ApiService {
  ...
  Future<Map<String, dynamic>> register(String nome, String pin, String question, String answer) async {
    final r = await http.post(Uri.parse('$_apiBase/auth/register'),
        headers: _h(), body: jsonEncode({'nome': nome, 'pin': pin, 'security_question': question, 'security_answer': answer}));
  ...
  Future<String> getSecurityQuestion(String nome) async {
    final r = await http.get(Uri.parse('$_apiBase/auth/security-question?nome=${Uri.encodeComponent(nome)}'));
    final d = jsonDecode(r.body) as Map<String, dynamic>;
    if (r.statusCode != 200) throw ApiException(d['detail'] ?? 'Usuário não encontrado');
    return d['security_question'] as String;
  }

  Future<Map<String, dynamic>> resetPin(String nome, String answer, String newPin) async {
    final r = await http.post(Uri.parse('$_apiBase/auth/reset-pin'),
        headers: _h(), body: jsonEncode({'nome': nome, 'security_answer': answer, 'new_pin': newPin}));
    final d = jsonDecode(r.body) as Map<String, dynamic>;
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  int     _recoveryStep   = 1;  // 1=buscar pergunta, 2=redefinir
  ...
  void _tapNum(String d) {
    if (_isRecovering) {
      ...
      setState(() { _recoveryPin += d; _recoveryError = null; });
    } else {
      if (_pin.length >= 4) return;
      setState(() { _pin += d; _error = null; });
      if (_pin.length == 4 && _nomeCtrl.text.trim().isNotEmpty && !_isCadastro) _submit();
    }
  }

  void _backspace() {
    if (_isRecovering) {
      if (_recoveryPin.isNotEmpty) setState(() => _recoveryPin = _recoveryPin.substring(0, _recoveryPin.length - 1));
    } else {
  ...
  Future<void> _fetchQuestion() async {
    ...
    if (nome.isEmpty) { setState(() => _recoveryError = 'Digite seu nome'); return; }
    setState(() { _recoveryLoading = true; _recoveryError = null; });
    try {
      final q = await _api.getSecurityQuestion(nome);
      setState(() { _recoveryQuestion = q; _recoveryStep = 2; _recoveryLoading = false; });
    } on ApiException catch (e) {
      setState(() { _recoveryError = e.message; _recoveryLoading = false; });
    } catch (_) {
      setState(() { _recoveryError = 'Sem conexão com o servidor'; _recoveryLoading = false; });
    }
  }
  ...
  Future<void> _doResetPin() async {
    ...
    if (answer.isEmpty)           { setState(() => _recoveryError = 'Digite a resposta'); return; }
    if (_recoveryPin.length < 4)  { setState(() => _recoveryError = 'Digite o novo PIN'); return; }
    setState(() { _recoveryLoading = true; _recoveryError = null; });
    try {
      final data = await _api.resetPin(nome, answer, _recoveryPin);
      widget.onLogin(data['token'] as String, data['nome'] as String);
    } on ApiException catch (e) {
      setState(() { _recoveryError = e.message; _recoveryPin = ''; _recoveryLoading = false; });
    } catch (_) {
      setState(() { _recoveryError = 'Sem conexão com o servidor'; _recoveryPin = ''; _recoveryLoading = false; });
    }
  }

  void _exitRecovery() => setState(() {
    _isRecovering = false; _recoveryStep = 1;
    _recoveryQuestion = ''; _recoveryPin = '';
    _recoveryError = null; _respostaCtrl.clear();
  });
  ...
  Future<void> _submit() async {
    ...
    if (_isCadastro) {
      if (_perguntaCtrl.text.trim().length < 5) { setState(() => _error = 'Pergunta de segurança muito curta'); return; }
      if (_respostaCtrl.text.trim().isEmpty)    { setState(() => _error = 'Digite a resposta de segurança'); return; }
    }
    setState(() { _loading = true; _error = null; });
    try {
      final data = _isCadastro
          ? await _api.register(nome, _pin, _perguntaCtrl.text.trim(), _respostaCtrl.text.trim())
  ...
  @override
  void dispose() { _nomeCtrl.dispose(); _perguntaCtrl.dispose(); _respostaCtrl.dispose(); super.dispose(); }
  ...
  Widget _pinDots(String pin) => Row(
    ...
    children: List.generate(4, (i) => Container(
      ...
      decoration: BoxDecoration(
        ...
        color: i < pin.length ? _blue : Colors.transparent,
        border: Border.all(color: i < pin.length ? _blue : _border, width: 1.5),
      ),
    )),
  );
                ...
                children: [
                  ...
                  if (_isCadastro) ...[
                    const Align(
                      ...
                      child: Text('SEGURANÇA', style: TextStyle(fontSize: 10, color: _muted, letterSpacing: 2)),
                    ),
                    ...
                    TextField(
                      ...
                      onChanged: (_) => setState(() => _error = null),
                      decoration: _fieldDeco('Pergunta de segurança (ex: nome do seu pet)'),
                    ),
                    const SizedBox(height: 8),
                    TextField(
                  ...
                  TextButton(
                    onPressed: () => setState(() { _isCadastro = !_isCadastro; _error = null; _pin = ''; _perguntaCtrl.clear(); _respostaCtrl.clear(); }),
                    child: Text(
                  ...
                  if (!_isCadastro)
                    TextButton(
                      onPressed: () => setState(() { _isRecovering = true; _nomeCtrl.clear(); }),
                      child: const Text('Esqueci o PIN', style: TextStyle(color: _muted, fontSize: 12)),
                    ),
                  const SizedBox(height: 20),
                ],
              ),
          ...
          child: ConstrainedBox(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  const SizedBox(height: 20),
                  const Icon(Icons.lock_reset_rounded, size: 36, color: _blue),
                  const SizedBox(height: 10),
                  const Text('RECUPERAR', style: TextStyle(fontSize: 11, letterSpacing: 4, color: _muted, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 4),
                  const Text('PIN', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: _txt, letterSpacing: -0.5)),
                  const SizedBox(height: 36),
                  ...
                  // Step 2: pergunta + resposta + novo PIN
                  if (_recoveryStep == 2) ...[
                    Container(
                      width: double.infinity,
                      ...
                      child: Text(_recoveryQuestion, style: const TextStyle(color: _dim, fontSize: 13)),
                    ),
                    const SizedBox(height: 10),
                    TextField(
                      controller: _respostaCtrl,
                    ...
                    const Text('NOVO PIN', style: TextStyle(fontSize: 10, color: _muted, letterSpacing: 2)),
                    const SizedBox(height: 12),
                  ...
                  if (_recoveryError != null) ...[
                    Text(_recoveryError!, style: const TextStyle(color: _red, fontSize: 12)),
                    const SizedBox(height: 12),
                  ],
                  ...
                  SizedBox(
                    ...
                    height: 48,
                    child: ElevatedButton(
                      onPressed: _recoveryLoading ? null : (_recoveryStep == 1 ? _fetchQuestion : _doResetPin),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: _blue,
                        foregroundColor: Colors.white,
                        elevation: 0,
                      ...
                      child: _recoveryLoading
                          ...
                          : Text(_recoveryStep == 1 ? 'BUSCAR PERGUNTA' : 'REDEFINIR PIN',
                              style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, letterSpacing: 1.5)),
                    ),
                  ),
                  const SizedBox(height: 14),
                  TextButton(
                    ...
                    child: const Text('Voltar ao login', style: TextStyle(color: _dim, fontSize: 12)),
                  ),
                  const SizedBox(height: 20),
                ],
              ),
